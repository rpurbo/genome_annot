...
=head1 Example
    perl ANI.pl -bl /apps/blast-2.2.25/bin/blastall -fd /apps/blast-2.2.25/bin/formatdb -qr $INPUT -sb $REF -od $OUTPUT 
=cut
...
`$bl -i $od/Query.split -d $od/Subject.fa -X 150 -q -1 -F F -e 1e-15 -m 8 -a 4 -o $od/raw.blast -p blastn`;
...
close BL;
$ANI = $sumID/$count;
open OFILE,">$od/ANI_results.out" or "$od/ANI_results.out $!\n";
print OFILE "$qr\t$sb\t$ANI";
close OFILE;
